...
//@}}}
    ...
    function zip(arrays) {
        var minLength = Math.min.apply(Math, map(arrays, function (xs) {
            return xs.length;
        }));
    ...
    var parseUriKeys = [ "source", "protocol", "authority", "userInfo", "user", "password", "host", "port", "relative", "path", "directory", "file", "query", "anchor" ];
    ...
    function loadResources(resourceHandles, config, callback) {
        var loadCallbacks = resourceHandles.map(function (handle) {
            return function (callback) {
    ...
    function handleDefine(args, config, callback) {
        ...
        announce(resourceHandle.id, function () {
            ...
            depResourceHandles.forEach(function (depResourceHandle) {
                addDependency(moduleName, depResourceHandle.id);
            });
            checkCycles();
