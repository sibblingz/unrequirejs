...
//@}}}
    ...
    function prepareExecute(deps, config, callback) {
        var weak = false;

        // AMD compliance: require
        var extraValues = [ ];
        if (deps[0] === 'require') {
            extraValues.push(makeRequire(config));
            deps = deps.slice(1);
            weak = true;
        }
        ...
        callback(null, moduleNames, weak, function (factory, values, callback) {
            if (typeof factory === 'function') {
                callback(null, factory.apply(null, extraValues.concat(values)));
            } else {
    ...
    function execute(deps, config, factory, doneCallback) {
        prepareExecute(deps, config, function (err, moduleNames, weak, callback2) {
            if (err) return doneCallback(err);

            loadModules(moduleNames, config, function (err, values) {
                callback2(factory, values, doneCallback);
            });
        });
    }
    ...
    function handleDefine(args, config, callback) {
        ...
        announce(moduleName, function () {
            ...
            var deps = args['deps'].slice();
            var extraValues = [ ];

            // AMD compliance: module and exports
            if (ENABLE_INNER_EXPORTS) {
                if (deps[1] === 'exports' && deps[2] === 'module') {
                    var exports = { };
                    var module = {
                        'exports': exports
                    };
                    ...
                    deps.splice(1, 2); // remove 'exports' and 'module'

                    // FIXME Should this push be deferred?
                    push(moduleName, exports);
                    pushed = true;
                }
            }
            ...
            prepareExecute(deps, config, function (err, moduleNames, weak, callback2) {
                ...
                if (weak) {
                    function tryBreakDeadlock(callback) {
                        if (doned) {
                            callback();
                            return;
                        }
                        ...
                        doned = true;
                        callback2(args['factory'], [ ], function (err, value) {
                            if (err) return callback(err);
                    ...
                    if (hasOwn(deadlockFunctions, moduleName)) {
                        deadlockFunctions[moduleName].push(callback);
                    } else {
                        deadlockFunctions[moduleName] = [ tryBreakDeadlock ];
                    }
                }

                moduleNames.forEach(function (dep) {
                ...
                loadModules(moduleNames, config, function (err, values) {
                    ...
                    doned = true;
                    callback2(args['factory'], extraValues.concat(values), function (err, value) {
                        if (err) return callback(err);
    ...
    function makeRequire(config) {
        return function(/* ... */) {
            var args = parseRequireArguments(arguments);
            return handleRequire(args, config, function (err) {
                err = getErrors(err);
                if (err) throw err;
