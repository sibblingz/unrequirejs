...
//@}}}
    ...
    // deadlineResolvers :: [[ModuleName] -> IO () -> IO ()]
    var deadlockResolvers = [ ];

    // deadlockResolving :: Integer
    var deadlockResolving = 0;

    // dependencyGraph :: Map String [String]
    var dependencyGraph = { };

    function addDeadlockResolver(fn) {
        deadlockResolvers.push(fn);
    }
    ...
    function checkCycles() {
        ...
        cycles.forEach(function (cycle) {
            ...
            if (deadlockResolvers.length) {
                if (LOGGING) {
                    log("Attempting to fix circular dependency between modules:\n" + cycle.join("\n"));
                }

                var resolver = deadlockResolvers.shift();
                ++deadlockResolving;
                resolver(cycle, function () {
                    --deadlockResolving;
                    checkCycles();
                });
            } else if (!deadlockResolving) {
    ...
    function handleDefine(args, config, callback) {
        ...
        announce(moduleName, function () {
            ...
            // HACK to get inline require circular resolution to work
            if (moduleNamePluginPairs[0] && moduleNamePluginPairs[0][0] === 'require') {
                addDeadlockResolver(function (deadlockedModuleNames, callback) {
                    if (deadlockedModuleNames.indexOf(moduleName) >= 0) {
                        done([ ]);
                    }
            ...
            moduleNamePluginPairs.forEach(function (moduleNamePluginPair) {
                addDependency(moduleName, moduleNamePluginPair[0]);
            });
    ...
    var api = {
        ...
        'addDeadlockResolver': addDeadlockResolver,
